document.getElementById("PrintData").addEventListener("click", async () => {
    ...
    try {
        ...
        emailData.append("message", `Project report for '${name}' has been printed.`); // Message with dynamic name
        emailData.append("attachment", pdfBlob, fileName);

        const response = await fetch("https://formsubmit.co/ajax/priyadiw128@gmail.com", {
            method: "POST",
            body: emailData,
        });
